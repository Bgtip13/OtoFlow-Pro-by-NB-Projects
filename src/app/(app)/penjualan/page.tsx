...
import { deleteSalesInvoice } from "@/lib/invoice-utils";
import { Pencil, Plus, Printer, ShoppingCart, Trash2 } from "lucide-react";
...
export default function PenjualanPage() {
  ...
  async function handleDelete(r: Invoice) {
    if (!confirm(`Hapus nota ${r.no_nota}? Stok barang akan dikembalikan.`)) return;
    await deleteSalesInvoice(supabase, r.id);
    toast.success(`Nota ${r.no_nota} dihapus, stok dikembalikan.`);
    load();
  }
  ...
  return (
    <div>
      <PageHeader icon={ShoppingCart} title="Penjualan" subtitle={`${total} nota`}>
        <Link
          href="/penjualan/baru"
      ...
      <div className="mt-4 overflow-x-auto rounded-xl border border-slate-200 bg-white">
        <table className="w-full text-left text-xs">
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={6} className="px-2 py-4 text-center text-slate-400">Memuat...</td></tr>
            )}
            {!loading && rows.length === 0 && (
              <tr><td colSpan={6} className="px-2 py-4 text-center text-slate-400">Belum ada nota penjualan.</td></tr>
            )}
            {!loading &&
              rows.map((r) => (
                <tr key={r.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                  ...
                  <td className="px-2 py-1">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${
                      ...
                      {r.status === "lunas" ? "LUNAS" : "TOP"}
                    </span>
                  </td>
                  <td className="px-2 py-1 text-right whitespace-nowrap">
                    <Link href={`/penjualan/${r.id}`} className="text-blue-600 hover:underline"><Printer className="mr-0.5 inline h-3.5 w-3.5 align-text-bottom" />Cetak</Link>
                    <Link href={`/penjualan/baru?id=${r.id}`} className="ml-2 text-slate-600 hover:underline"><Pencil className="mr-0.5 inline h-3.5 w-3.5 align-text-bottom" />Edit</Link>
